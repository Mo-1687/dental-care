...
function Navbar() {
  ...
  const navItems = user
    ? navItemsForLoggedUser.map((item) => (
        <Link
          key={item.href}
          href={item.href}
    ...
    : navItemsForUnLoggedUser.map((item, index) => (
        <a
          key={index}
          href={item.href}
          className={`flex items-center gap-2 transition-colors ${
  ...
  return (
    <nav className="fixed top-0 w-screen z-50 px-6 py-2 border-b border-border/50 bg-background/80 backdrop-blur-md h-16">
      <div className="max-w-7xl mx-auto flex justify-between items-center h-full">
        ...
        <div className="lg:flex hidden items-center gap-6">{navItems}</div>
